...
- **2026-05-17:** Extended the annual DCA scenario to support 5/10/15/20-year horizons (was 10-year only). Total contributions scale with horizon — $500 at 5 years, $1,000 at 10, $1,500 at 15, $2,000 at 20 — preserving the "$100 annual" branding. Introduced a `total_invested(horizon)` method on the Scenario classes (default returns `meta.total_invested`; `AnnualDCA100` overrides to return `DEPOSIT_AMOUNT × horizon`). `src/export.py` now emits a per-horizon `total_invested` map alongside the rolling windows; `docs/app.js` reads it (falling back to the scenario-level value) so the break-even reference, terminal-chart shading, and stats-table thresholds all update when the user switches horizons on the DCA page. Updated scenario descriptions, card blurbs, and comparison-note callouts to be horizon-agnostic. PDF report still builds at the 10-year horizon for both scenarios (where DCA total = $1,000 = `meta.total_invested`, so no change in output).
- **2026-05-16:** Visual redesign of the static site. Replaced the Pico-classless default look with a custom layout: sticky branded header with inline SVG mark and primary navigation (Scenarios / About), a hero band on the landing page, a scenario card grid (auto-fit minmax(280px, 1fr)) with hover lift + animated brand-color top accent, a three-column footer (about / scenarios / resources) collapsing to two then one column on narrow viewports, breadcrumb + "Download PDF" pill bar on scenario pages, and a new About page. CSS variables centralize the palette (`--brand-color`, `--brand-bg-tint`, `--negative-color`). Responsive breakpoints at 900/720/480px; chart heights now use `clamp(280px, 48vw, 420px)`. Pages are forced to `data-theme="light"` for visual consistency. Negative-coloring conventions (red shading and `.negative` class) preserved.
- **2026-05-16:** Added red coloring for negative outcomes — annualized returns below zero and terminal values below the break-even contribution — to every chart and table on the site and in the PDFs. Site: red fill under zero on the rolling-metric line chart via a clamped `tozeroy` trace plus red markers; manually-binned bar histograms with negative bins recolored; red `tonexty` fill on the terminal-value chart between line and break-even reference; `.negative` CSS class with `--negative-color: #cc3333` for stats-table cells. PDFs: `fill_between(... where=(y<threshold))` shading on both Nominal and Real lines, negative-bin recoloring via patch iteration, and red bold text in the best/worst tables. `NEG_COLOR` / `NEG_FILL_ALPHA` constants in `src/report_template.py`. Also tightened the Nominal/Real labeling: every chart title, axis, and panel now explicitly names the return type instead of relying on the dropdown context. Conventions documented in `CLAUDE.md` § Charting conventions and `design.md` § PDF report layout.
- **2026-05-16:** Branded the published deliverable (site + PDFs) under the fictitious name **Fubar Analytics** with the tagline "Long-run market research" and an accent color `#2c5f7a`. Brand identity is centralized in two places: `BRAND_NAME` / `BRAND_TAGLINE` / `BRAND_COLOR` constants at the top of `src/report_template.py` for the PDFs, and the matching `.brand-header` / `--brand-color` rules in `docs/style.css` for the site. Each HTML page has a brand-strip header and a footer copyright line; each PDF has a brand strip above the cover title and the brand listed in the page footer + PDF Author metadata. Changing the brand name in those two locations propagates everywhere.
- **2026-05-16:** Moved per-scenario PDFs from the gitignored project-root `reports/` to a committed `docs/reports/<slug>.pdf` so GitHub Pages serves them. The landing page and each scenario page now show a "Download PDF report" link. Added `scripts/publish.ps1` as the canonical refresh-and-review workflow (runs `src.ingest` if the parquet is missing, then `src.export`, then `src.report`, then prints `git status` for `docs/`). Tightened the `.gitignore` rule from `reports/` to `/reports/` so it only matches the legacy project-root path, not the new committed location.
